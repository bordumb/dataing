...
This module provides an Amazon Redshift adapter that implements the unified
...
REDSHIFT_CONFIG_SCHEMA = ConfigSchema(
    field_groups=[
...
REDSHIFT_CAPABILITIES = AdapterCapabilities(
    supports_sql=True,
...
@register_adapter(
    ...
    icon="redshift",
    description="Connect to Amazon Redshift data warehouses",
    capabilities=REDSHIFT_CAPABILITIES,
    config_schema=REDSHIFT_CONFIG_SCHEMA,
)
class RedshiftAdapter(SQLAdapter):
    """Amazon Redshift database adapter.

    Provides full schema discovery and query execution for Redshift clusters.
    Uses asyncpg for connection as Redshift is PostgreSQL-compatible.
    """
        ...
        try:
            ...
            if "password" in error_str or "authentication" in error_str:
                raise AuthenticationFailedError(
                    message="Authentication failed for Redshift",
                    details={"error": str(e)},
                ) from e
            elif "timeout" in error_str:
                raise ConnectionTimeoutError(
                    message="Connection to Redshift timed out",
                    timeout_seconds=self._config.get("connection_timeout", 30),
                ) from e
            else:
                raise ConnectionFailedError(
                    message=f"Failed to connect to Redshift: {str(e)}",
                    details={"error": str(e)},
                ) from e
    ...
    async def get_schema(
        ...
        try:
            ...
            pk_sql = """
                ...
                JOIN pg_attribute a ON ti.table_id = a.attrelid
                WHERE a.attnum > 0
            ...
            for row in columns_result.rows:
                ...
                if schema_name in schema_map and table_name in schema_map[schema_name]:
                    ...
                    col_data = {
                        ...
                        "data_type": normalize_type(row["data_type"], SourceType.REDSHIFT),
            ...
            return self._build_schema_response(
                source_id=self._source_id or "redshift",
                catalogs=catalogs,
            )

        except Exception as e:
            raise SchemaFetchFailedError(
                message=f"Failed to fetch Redshift schema: {str(e)}",
                details={"error": str(e)},
            ) from e

    def _build_sample_query(self, table: str, n: int) -> str:
        """Build Redshift-specific sampling query."""
